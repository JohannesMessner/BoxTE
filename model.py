...
class BaseBoxE(nn.Module):
    ...
    def __init__(
        ...
        timestamps: Any,
        device: str = "cpu",
        weight_init_args: Tuple[float, float] = (0, 1),
        norm_embeddings: bool = False,
    ):
        """
        :param embedding_dim: embedding dimensionality
        :param relation_ids: list of relation ids.
            `entity_ids + relation_ids` must be a sorted list of ascending ints starting from 0
        :param entity_ids: list of entity ids.
            `entity_ids + relation_ids` must be a sorted list of ascending ints starting from 0
        :param timestamps: list (?) of timestamps
        :param device: torch device to use for computation
        :param weight_init_args: tuple of args to pass to torch.nn.init.uniform_
        :param norm_embeddings: whether to normalize embeddings
        """
        super().__init__()
        ...
        # it is assumed that relation_ids "come after" entity ids
        ...
        assert (
            sorted(entity_ids + relation_ids) == entity_ids + relation_ids
        ), "ids need to be ascending ints from 0 with entities coming before relations"

        # relation embeddings
        # TODO: combine some of these embeddings into one
    ...
    def shape_norm(self, t: torch.Tensor, dim: int) -> torch.Tensor:
    ...
    def to(self: T, device: Union[str, torch.device]) -> T:
        ...
        :param device: torch device to move model to
        :return: self
        """
        super().to(device)
        self.device = device
    ...
    def get_relation_idx_by_id(self, r_ids: torch.Tensor):
        """
        Get indices of relations in embedding tensors by relation ids
        :param r_ids: tensor of relation ids
        :return: tensor of indices
        """
        return r_ids - self.relation_id_offset

    def get_e_idx_by_id(self, e_ids: torch.Tensor):
        """
        Get indices of entities in embedding tensors by entity ids
        :param e_ids: tensor of entity ids
        :return: tensor of indices
        """
        return e_ids

    def compute_relation_embeddings(self, tuples: torch.Tensor) -> torch.Tensor:
        """
        Compute relation embeddings for given tuples (head, relation, tail, time)
        :param tuples: tensor of tuples of shape (nb_examples, 4, batch_size)
        :return: tensor of relation embeddings of shape (nb_examples, batch_size, arity, 2, embedding_dim)
        """
        # TODO: verify the shape in the docstrings above
        nb_examples, _, batch_size = tuples.shape
        rel_idx = self.get_relation_idx_by_id(tuples[:, 1]).to(self.device)
        # get relevant embeddings
        r_head_bases = self.r_head_base_points(rel_idx)
        r_tail_bases = self.r_tail_base_points(rel_idx)

        r_head_widths = self.shape_norm(
            self.r_head_widths(rel_idx), dim=2
        )  # normalize relative widths
        r_tail_widths = self.shape_norm(self.r_tail_widths(rel_idx), dim=2)

        r_head_scales = (
            nn.functional.elu(self.r_head_size_scales(rel_idx)) + 1
        )  # ensure scales > 0
    ...
    def compute_entity_embeddings(self, tuples: torch.Tensor) -> torch.Tensor:
        """
        Compute entity embeddings for given tuples (head, relation, tail, time)
        :param tuples: tensor of tuples of shape (nb_examples, 4, batch_size)
        :return: tensor of entity embeddings of shape (nb_examples, batch_size, arity, embedding_dim)
        """
        ...
        return self.embedding_norm_fn(
            torch.stack((head_bases + tail_bumps, tail_bases + head_bumps), dim=2)
        )

    def compute_embeddings(self, tuples: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Compute entity and relation embeddings for given tuples (head, relation, tail, time)
        :param tuples: tensor of tuples of shape (nb_examples, 4, batch_size)
        :return: tuple (entities, relations) containing embeddings with shape
            entities.shape = (nb_examples, batch_size, arity, embedding_dim)
            relations.shape = (nb_examples, batch_size, arity, 2, embedding_dim)
        """
        # get relevant entity bases and bumps
        return self.compute_entity_embeddings(tuples), self.compute_relation_embeddings(
            tuples
        )

    def forward_negatives(self, negatives: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Compute entity and relation embeddings for given negative tuples (head, relation, tail, time)

        :param negatives: tensor of negative tuples of shape (nb_negative_samples, 4, batch_size)
    ...
    def forward_positives(self, positives: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Compute entity and relation embeddings for given positive tuples (head, relation, tail, time)
        :param tuples: tensor of tuples of shape (nb_examples, 4, batch_size)
    ...
    def forward(self, positives: torch.Tensor, negatives: torch.Tensor) -> Tuple[Tuple[torch.Tensor, torch.Tensor], Tuple[torch.Tensor, torch.Tensor]]:
        """
        Compute entity and relation embeddings for given positive and negative tuples (head, relation, tail, time)
...
class TBoxE(BaseBoxE):
    ...
    def __init__(
        ...
        self.time_head_base_points = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, self.embedding_dim))
        )
        self.time_head_widths = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, self.embedding_dim))
        )
        self.time_head_size_scales = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, 1))
        )
        self.time_tail_base_points = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, self.embedding_dim))
        )
        self.time_tail_widths = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, self.embedding_dim))
        )
        self.time_tail_size_scales = nn.Parameter(
            torch.empty((self.max_time, self.nb_relations, 1))
        )
    ...
    def compute_time_embeddings(self, tuples):
        ...
        time_head_widths = self.shape_norm(
            self.time_head_widths[time_idx, rel_idx, :], dim=2
        )  # normalize relative widths
        time_tail_widths = self.shape_norm(
            self.time_tail_widths[time_idx, rel_idx, :], dim=2
        )

        time_head_scales = (
            nn.functional.elu(self.time_head_size_scales[time_idx, rel_idx, :]) + 1
        )  # ensure scales > 0
        time_tail_scales = (
            nn.functional.elu(self.time_tail_size_scales[time_idx, rel_idx, :]) + 1
        )
        # compute scaled widths
...
class LegacyBoxTE(BaseBoxE):
    def __init__(
        ...
        use_e_factor=False,
        nb_timebumps=1,
        use_r_rotation=False,
        use_e_rotation=False,
        nb_time_basis_vecs=-1,
        norm_time_basis_vecs=False,
        use_r_t_factor=False,
        dropout_p=0.0,
        arity_spec_timebumps=False,
    ):
        super().__init__(
            embedding_dim,
            relation_ids,
        ...
        self.use_r_factor, self.use_e_factor, self.use_r_t_factor = (
            use_r_factor,
            use_e_factor,
            use_r_t_factor,
        )
        ...
        if (
            not self.nb_time_basis_vecs > 0
        ):  # don't factorize time bumps, learn them directly/explicitly
            self.factorize_time = False
            if self.arity_spec_timebumps:
                self.head_time_bumps = nn.Parameter(
                    torch.empty(self.max_time, self.nb_timebumps, self.embedding_dim)
                )
                self.tail_time_bumps = nn.Parameter(
                    torch.empty(self.max_time, self.nb_timebumps, self.embedding_dim)
                )
                self.init_f(self.head_time_bumps, *weight_init_args)
                self.init_f(self.tail_time_bumps, *weight_init_args)
            else:
                self.time_bumps = nn.Parameter(
                    torch.empty(self.max_time, self.nb_timebumps, self.embedding_dim)
                )
            ...
            if self.arity_spec_timebumps:
                self.head_time_bumps_a = nn.Parameter(
                    torch.empty(
                        self.nb_timebumps, self.max_time, self.nb_time_basis_vecs
                    )
                )
                self.head_time_bumps_b = nn.Parameter(
                    torch.empty(
                        self.nb_timebumps, self.nb_time_basis_vecs, self.embedding_dim
                    )
                )
                ...
                self.time_bumps_a = nn.Parameter(
                    torch.empty(
                        self.nb_timebumps, self.max_time, self.nb_time_basis_vecs
                    )
                )
                self.time_bumps_b = nn.Parameter(
                    torch.empty(
                        self.nb_timebumps, self.nb_time_basis_vecs, self.embedding_dim
                    )
                )
        ...
        if self.use_r_factor:
            if self.arity_spec_timebumps:
                self.head_r_factor = nn.Parameter(
                    torch.empty(self.nb_relations, self.nb_timebumps, 1)
                )
                self.tail_r_factor = nn.Parameter(
                    torch.empty(self.nb_relations, self.nb_timebumps, 1)
                )
                torch.nn.init.normal_(self.head_r_factor, 1, 0.1)
                torch.nn.init.normal_(self.tail_r_factor, 1, 0.1)
            else:
                self.r_factor = nn.Parameter(
                    torch.empty(self.nb_relations, self.nb_timebumps, 1)
                )
        ...
        if self.use_r_t_factor:
            self.r_t_factor = nn.Parameter(
                torch.empty(self.nb_relations, self.max_time, self.nb_timebumps, 1)
            )
            torch.nn.init.normal_(self.r_t_factor, 1, 0.1)
        if self.use_e_factor:
            self.e_factor = nn.Parameter(
                torch.empty(self.nb_entities, self.nb_timebumps, 1)
            )
            torch.nn.init.normal_(self.e_factor, 1, 0.1)
        if self.use_r_rotation:
            self.r_angles = nn.Parameter(
                torch.empty(self.nb_relations, self.nb_timebumps, 1)
            )
            torch.nn.init.normal_(self.r_angles, 0, 0.1)
        if self.use_e_rotation:
            self.e_angles = nn.Parameter(
                torch.empty(self.nb_entities, self.nb_timebumps, 1)
            )
    ...
    def dropout_timebump(self, bumps):
        ...
        if (
            self.training
        ):  # undo dropout scaling, not needed here since we're using a mask
    ...
    def compute_combined_timebumps(self, ignore_dropout=False):
        if not self.arity_spec_timebumps:
            ...
            head_bumps = self.compute_timebumps(
                is_tail=False, ignore_dropout=ignore_dropout
            )
            tail_bumps = self.compute_timebumps(
                is_tail=True, ignore_dropout=ignore_dropout
            )
    ...
    def apply_rotation(self, vecs, angles):
        ...
        angles = torch.cat(
            [angles for _ in range(emb_dim - 1)], dim=3
        )  # same angle for all dims
        vecs_sph = vecs.view(
            flat_shape
        )  # interpret given vectors as spherical coordinates
        vecs_sph[:, 1:] = to_angle_interval(
            vecs_sph[:, 1:]
        )  # angles need to be in [0, 2pi)
        vecs_sph[:, 1:] += angles.view(
            (nb_examples * batch_size * nb_timebumps, emb_dim - 1)
        )  # apply angles
        vecs_sph[:, 1:] = to_angle_interval(
            vecs_sph[:, 1:]
        )  # angles need to be in [0, 2pi)
    ...
    def compute_embeddings(self, tuples):
        ...
        time_vecs_h, time_vecs_t = time_vecs_h.sum(dim=2), time_vecs_t.sum(
            dim=2
        )  # sum over all time bumps
        time_vecs = torch.stack(
            (time_vecs_h, time_vecs_t), dim=2
        )  # apply to both heads and tails
        entity_embs = entity_embs + self.time_weight * time_vecs
        return (
            self.embedding_norm_fn_(entity_embs),
            self.embedding_norm_fn_(relation_embs),
            None,
        )
...
class DEBoxE(BaseBoxE):
    def __init__(
        ...
        self.time_features_mask = torch.arange(
            self.embedding_dim, device=self.device
        ) < (time_proportion * self.embedding_dim)
        self.time_w = nn.Parameter(
            torch.empty((self.nb_entities, embedding_dim), device=self.device)
        )
        self.time_b = nn.Parameter(
            torch.empty((self.nb_entities, embedding_dim), device=self.device)
        )
